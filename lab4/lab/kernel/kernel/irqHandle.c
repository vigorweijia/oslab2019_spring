...
void syscallSemWait(struct StackFrame *sf) { //P(s)
	int i = sf->edx;
	sem[i].value--;
	/*putStr("wait semaphore: ");
	putNum(sem[i].value);
	putChar('\n');*/
	if(sem[i].value < 0) {
		//insert into wating list
		struct ListHead *nowhead = sem[i].pcb.next;
		sem[i].pcb.next = &pcb[current].blocked;
		nowhead->prev = &pcb[current].blocked;
		pcb[current].blocked.prev = &sem[i].pcb;
		pcb[current].blocked.next = nowhead;
		/*putStr("&sem[i].pcb: ");
		putNum((uint32_t)&sem[i].pcb);
		putChar('\n');
		putStr("sem[i].pcb.next: ");
		putNum((uint32_t)sem[i].pcb.next);
		putChar('\n');
		putStr("sem[i].pcb.prev: ");
		putNum((uint32_t)sem[i].pcb.prev);
		putChar('\n');
		putStr("pcb[current].pid addr:");
		putNum((uint32_t)&pcb[current].pid);
		putChar('\n');
		putStr("pcb[current].blocked addr:");
		putNum((uint32_t)&pcb[current].blocked);
		putChar('\n');
		putStr("pcb[current].blocked.next: ");
		putNum((uint32_t)pcb[current].blocked.next);
		putChar('\n');
		putStr("pcb[current].blocked.prev: ");
		putNum((uint32_t)pcb[current].blocked.prev);
		putChar('\n');*/
		//block self
		pcb[current].state = STATE_BLOCKED;
		pcb[current].sleepTime = -1; // distinguish from sleep operation
...
void syscallSemPost(struct StackFrame *sf) { //V(s)
	int i = sf->edx;
	sem[i].value++;
	/*putStr("post semaphore: ");
	putNum(sem[i].value);
	putChar('\n');*/
	if(sem[i].value <= 0) {
		//release from waiting list
		//if(sem[i].pcb.prev == &sem[i].pcb) assert(0);
		struct ListHead temp = *sem[i].pcb.prev;
		struct ListHead *addr = sem[i].pcb.prev;
		/*putStr("addr: ");
		putNum((uint32_t)addr);
		putChar('\n');*/
		uint32_t pid_addr_num = (uint32_t)addr - 36;
		uint32_t *pid_adrr = (uint32_t*)pid_addr_num;

		sem[i].pcb.prev = temp.prev;
		(*temp.prev).next = &sem[i].pcb;
		
		int release_id = *pid_adrr;
		/*putStr("Released id:");
		putNum((uint32_t)*pid_adrr);
		putChar('\n');*/

		pcb[release_id].state = STATE_RUNNABLE;
		pcb[release_id].timeCount = 0;
		pcb[release_id].sleepTime = 0;
